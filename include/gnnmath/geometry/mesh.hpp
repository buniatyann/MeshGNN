#ifndef GNNMATH_GEOMETRY_MESH_HPP
#define GNNMATH_GEOMETRY_MESH_HPP
...
public:
    ...
    struct face_with_attrs {
        ...
        std::array<std::optional<std::size_t>, 3> texcoord_indices;
        std::array<std::optional<std::size_t>, 3> normal_indices;
    };
    ...
    /**
     * @brief Loads a mesh from an OBJ file with custom options.
     * @param filename Path to the OBJ file.
     * @param options Loading options.
     * @throws std::runtime_error If the file cannot be opened or contains invalid data.
     */
    void load_obj(const std::string& filename, const obj_load_options& options);

    /**
     * @brief Loads a mesh from pre-parsed OBJ data.
     * @param data Parsed OBJ data from obj_loader.
     */
    void load_from_obj_data(const obj_data& data);
...
public:
    ...
    /// @brief Accesses the texture coordinates (mutable).
    /// @return Reference to the texture coordinates vector.
    std::vector<texcoord>& texcoords() { return texcoords_; }

    /// @brief Accesses the texture coordinates (immutable).
    /// @return Const reference to the texture coordinates vector.
    const std::vector<texcoord>& texcoords() const { return texcoords_; }

    /// @brief Accesses the file normals (mutable).
    /// @return Reference to the file normals vector.
    std::vector<normal>& file_normals() { return file_normals_; }

    /// @brief Accesses the file normals (immutable).
    /// @return Const reference to the file normals vector.
    const std::vector<normal>& file_normals() const { return file_normals_; }

    /// @brief Accesses the faces with full attributes (mutable).
    /// @return Reference to the faces with attributes vector.
    std::vector<face_with_attrs>& faces_with_attrs() { return faces_with_attrs_; }

    /// @brief Accesses the faces with full attributes (immutable).
    /// @return Const reference to the faces with attributes vector.
    const std::vector<face_with_attrs>& faces_with_attrs() const { return faces_with_attrs_; }

    /// @brief Check if the mesh has texture coordinates.
    /// @return True if texture coordinates are present.
    bool has_texcoords() const { return !texcoords_.empty(); }

    /// @brief Check if the mesh has file normals.
    /// @return True if file normals are present.
    bool has_file_normals() const { return !file_normals_.empty(); }
...
#endif // GNNMATH_GEOMETRY_MESH_HPP
